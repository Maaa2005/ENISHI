...
from twinlink_core.models import CloneAgent, CloneStatus, MemoryItem, MemorySnapshot, User
from twinlink_core.services import memory_sources
from twinlink_core.services.audit import log_event
from twinlink_core.services.memories import exportable_memories
from twinlink_core.services.projects import get_project
...
def _store_memory_snapshot(
    ...
    """クローン生成時に使用した記憶IDを保存する（twinlink.md §17）。"""
...
def build_clone_draft(
    ...
    from twinlink_core.services.clones import DEFAULT_CODING_PROFILE
...
def rebuild_clone(session: Session, clone_id: str) -> CloneAgent:
    """既存クローンを最新の記憶で再生成する（twinlink.md §30 /rebuild）。"""
