"""外部コーディングエージェントタスクサービス（twinlink.md §23, §31）。"""
...
from twinlink_core.providers.base import CodingTaskResult
...
def get_task(session: Session, task_id: str) -> CodingTask:
    ...
    if task.approval_id is not None:
        from twinlink_core.services.approvals import get_approval
...
def recover_interrupted_tasks(session: Session) -> int:
    """起動時に残ったrunning/cancellingタスクをfailedへ倒す（twinlink.md §30）。"""
    now = utc_now()
...
def claim_next_task(session: Session, worker_id: str) -> CodingTask | None:
    """queuedタスクを条件付きUPDATEで1件だけclaimする（twinlink.md §30）。"""
...
async def worker_loop(stop_event: asyncio.Event) -> None:
    """単一ワーカー・並行数1でCodingTaskキューを処理する（twinlink.md §30）。"""
