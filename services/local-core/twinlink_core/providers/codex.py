"""Codex CLIアダプター（twinlink.md §21）。"""

from pathlib import Path

from twinlink_core.models import CloneContextPackage
from twinlink_core.providers.base import (
    AuthenticationStatus,
    CodingTaskResult,
    ProviderCapabilities,
    ProviderDetectionResult,
)
from twinlink_core.providers.cli_runner import run_cli, run_cli_async
from twinlink_core.services.environment import detect_provider
...
class CodexCliAdapter:
    ...
    async def check_authentication(self) -> AuthenticationStatus:
        help_code, help_output = run_cli(["codex", "--help"], timeout=10)
        if help_code != 0 or "login" not in help_output:
            return AuthenticationStatus(
                provider=self.provider,
                authenticated=None,
                detail="authentication status is unknown",
            )
        status_code, _status_output = run_cli(["codex", "login", "status"], timeout=10)
        return AuthenticationStatus(
            provider=self.provider,
            authenticated=status_code == 0,
            detail="login status command completed",
        )

    async def get_capabilities(self) -> ProviderCapabilities:
        help_code, _help_output = run_cli(["codex", "--help"], timeout=10)
    ...
    async def run_task(
        ...
        context_path = project_root / f".twinlink-context-{context_package.id}.json"
