"""Relay Serverクライアント（enishi.md §25 Relay Serverの責務 v2）。
...
def _unavailable(exc: Exception) -> EnishiError:
    return EnishiError(
...
def get_relay_client() -> RelayClient:
    ...
    if not settings.relay_url or not settings.relay_token:
        raise EnishiError(
            code="RELAY_UNAVAILABLE",
            message="Relayの接続設定（ENISHI_RELAY_URL / ENISHI_RELAY_TOKEN）がありません。",
            status_code=503,
        )
