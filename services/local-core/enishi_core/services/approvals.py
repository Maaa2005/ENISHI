...
from enishi_core.models import Approval, ApprovalStatus
...
def expire_if_needed(session: Session, approval: Approval) -> Approval:
    """期限切れpending承認をexpiredへ遅延遷移する（enishi.md §31）。"""
    ...
    if approval.action_type == "negotiation_decision" and approval.payload.get("remote"):
        from enishi_core.services.remote_negotiation import resolve_remote_approval
    ...
    from enishi_core.services.tasks import expire_task_for_approval

    expire_task_for_approval(session, approval)
    from enishi_core.services.negotiation import on_approval_resolved
