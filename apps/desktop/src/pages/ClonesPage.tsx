...
export function ClonesPage({ client }: { client: ApiClient | null }) {
  ...
  useEffect(() => {
    ...
    setError(null);
    void Promise.all([client.listClones(users[0].id), client.listMemories(users[0].id)]).then(([cloneRows, memoryRows]) => {
      setClones(cloneRows);
  ...
  return (
    <main className="page clones-page">
      ...
      {!error && clones.length === 0 && <section className="empty-state"><div className="empty-icon">◉</div><h2>クローンはまだありません</h2><p>代理AI設定を完了すると、確認待ちのクローンが生成されます。</p></section>}
      {selected && <div className="clone-layout"><aside className="clone-list"><div className="session-panel-header"><h2>バージョン</h2><span>{clones.length}件</span></div>{clones.map((clone) => <button key={clone.id} onClick={() => setSelectedId(clone.id)} className={`clone-item ${clone.id === selected.id ? "selected" : ""}`}><span className={`session-status ${clone.status === "active" ? "agreed" : clone.status === "outdated" ? "failed" : "waiting"}`} /><div><strong>{clone.name}</strong><p>v{clone.version}・{statusLabel(clone.status)}</p></div></button>)}</aside><section className="clone-detail"><div className="clone-hero"><div className="confidence-ring" style={{ background: `conic-gradient(#3f8058 ${score * 3.6}deg, #e4e5e7 0deg)` }}><div><strong>{score}%</strong><span>確信度</span></div></div><div><p className="eyebrow">ACTIVE PERSONAL AGENT</p><h2>{selected.name}</h2><p>{memories.length}件の記憶と本人の委任方針を使い、必要なときだけ確認を求めます。</p><div className="clone-tags"><span>署名付き通信</span><span>選択的開示</span><span>人間承認ゲート</span></div></div></div><dl className="clone-profile">{profileSummary(selected).map((item) => <div key={item.label}><dt>{item.label}</dt><dd>{item.value}</dd></div>)}</dl><section className="clone-boundary"><div><strong>このクローンに任せられること</strong><p>信頼済みの相手との日程候補の比較・交渉・合意案の作成</p></div><div><strong>本人確認が必要なこと</strong><p>関係性ルールに該当する合意、低確信度の判断、外部へ影響する操作</p></div><div><strong>禁止されていること</strong><p>秘密情報の送信、破壊的操作、本人の許可を越えた確定</p></div></section></section></div>}
    </main>
  );
}
