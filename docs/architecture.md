...
ENISHI Local Core (FastAPI, 127.0.0.1 only)
  ...
  └─ /v1/*  (bearer auth required)
       └─ SQLite (~/Library/Application Support/ENISHI/enishi.db)
```

- Tauri launches the Local Core as a child process and kills it on exit (`src-tauri/src/process/`).
- The auth token is generated per session and passed to the Local Core via the `ENISHI_LOCAL_TOKEN` environment variable.
